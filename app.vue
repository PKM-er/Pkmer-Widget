 ...
 * @LastEditTime: 2023-11-08 20:46:23
 * @Description: 
-->
...
onMounted(async () => {
  ...
  if (userInfo.value) {
    ...
    if (url.pathname.includes("/pro/") && userInfo.value.type != "insider") {
      isPro.value = true;
    }
  } else {
    isLogin.value = false;
...
<template>
  <div v-if="isLogin !== null">
    ...
    <div v-else-if="isLogin && isPro">
      <!-- 越权访问提示 -->
      <div>
        此挂件为收费挂件，请到<a href="https://pkmer.cn">pkmer</a>升级会员级别。
      </div>
    </div>
    <div v-else-if="isLogin && isExpir">
      <!-- 到期提示 -->
      <div>
        会员权益已到期，请到<a href="https://pkmer.cn">pkmer</a>进行延期。
      </div>
    </div>
    <div v-else>
      <!-- 未登录提示 -->
      <div>
        请到<a href="https://pkmer.cn">pkmer</a>免费注册会员以使用此挂件！
      </div>
    </div>
  </div>
</template>
