...
const week_ZN = ['星期日', '星期一', '星期二', '星期三', '星期四', '星期五', '星期六'];
...
onMounted(() => {
	...
	const storedSelectedDate = localStorage.getItem('CountdownCards-selectedDate');
	...
	<div class="sub-container">
		<div class="days-box">
			...
			>
				...
				<span v-if="!isEditing && days > 0" @click="handleClick"> {{ displayValue }}还剩</span>
				<span v-if="!isEditing && days <= 0" @click="handleClick"> {{ displayValue }}已过</span>
			</div>
			<div class="content" id="number">{{ Math.abs(days) }}</div>

			<div v-if="!showPicker && days > 0" class="bottom" id="date" @click="handleClickDate">
...
.days-box:before, .days-box:after {
    content: '';
    position: absolute;
    top: -7px;
    width: 4px;
    height: 14px;
    background: -webkit-gradient(linear, left top, left bottom, from(#f1f1f1), to(rgb(177, 172, 172)));
}
.head {
	width: 100%;
	height: 50px;
