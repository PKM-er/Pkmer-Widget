...
watchEffect(() => {
	...
	if (props.widgetConfig['layout-style'] == 'Vertical') {
		themeColor_G.value = tinycolor(themeColor_B.value).lighten(50).setAlpha(0.1).toString();
		themeColor_D.value = tinycolor(themeColor_B.value).darken(15).desaturate(10).toString();
	}
	themeColor_L.value = tinycolor(themeColor_B.value).lighten(20).setAlpha(0.5).toString();
...
const getMusic = () => {
	let left = checkMusic();
	if (!left) {
		music.value = JSON.parse(localStorage.left).music[Math.floor(Math.random() * 10)];
		return setAudio();
	}
	axios
		.get(`https://api.uomg.com/api/rand.music?sort=%E7%83%AD%E6%AD%8C%E6%A6%9C&format=json`)
		.then((res) => {
...
const remainingTimeFormatted = computed(() => formatSecond(duration.value - current_time.value));
