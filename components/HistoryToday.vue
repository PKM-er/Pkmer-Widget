 ...
 * @LastEditTime: 2023-03-20 11:40:08
 * @Description: 
-->
...
watchEffect(() => {
	...
	themeColor_L.value = tinycolor(themeColor_B.value).darken(15).greyscale().toString();
	themeColor_D.value = tinycolor(themeColor_B.value).darken(35).brighten(20).toString();
});
...
const getHisToday = async (month: string, day: string) => {
	...
	try {
		const res = await axios.get(`https://baike.baidu.com/cms/home/eventsOnHistory/${month}.json`);
